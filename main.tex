...
\begin{abstract}
A Variabilidade da Frequência Cardíaca (da sigla em inglês, HRV) é uma medida fisiológica associada ao Sistema Nervoso Autônomo e suas reações a estímulos de estresse no ambiente. Nesse trabalho, investigamos a viabilidade de usá-la como um marcador biológico para identificar atividades diárias. Desenvolvemos um pipeline para extrair e processar o sinal obtido de forma não-invasiva, preparando-o para análise em classificadores. Derivamos um classificador para atividades e propomos métodos de biofeedback para que um indivíduo seja capaz de, voluntariamente, elevar ou reduzir parâmetros em sua própria HRV, o que nos permitiria detectar sua intenção utilizando classificador semelhante ao de atividades. Visamos desenvolver, dessa forma, uma interação humano-máquina que pode, em algumas situações, ser utilizada no lugar de métodos baseados em EEG, cujos equipamentos são mais complexos e caros, ou juntamente com esses métodos, para fornecer contexto a eles.


\paragraph{}
\textit{Supervised Classifier of Activities and Commands \\
